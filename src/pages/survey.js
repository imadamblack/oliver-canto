...
───────────────────────────────────────── */
const WHATSAPP_NUMBER = 'PENDIENTE'; // ej: 522221234567
...
const Intro = ({ onButtonClick }) => (
  ...
  >
    ...
    >
      ...
      />
      <div className="absolute inset-0 bg-gradient-to-b from-[#1a0a00] via-[#1a0a00]/80 to-transparent pointer-events-none z-10" />
      <div className="absolute inset-0 bg-[url('/survey/00.jpg')] bg-cover bg-center pointer-events-none -z-10" />
      ...
      <div className="container flex flex-col mx-auto relative h-full z-10">
        <div className="max-w-2xl flex flex-col flex-grow">
          ...
          {/* Headline */}
          <h1 className="ft-6 font-black text-white tracking-tight mb-5">
            Diseño el restaurante que tienes en la cabeza. Y el negocio que lo sostiene.
          </h1>

          {/* Subtítulo */}
          <p className="ft-2 text-neutral-300 mb-8 leading-relaxed flex-grow">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <a
              className="button !bg-brand-5 ft-1"
...
const formSteps = () => ([
  ...
  // P4 — Inversión contemplada
  {
    type: 'radio',
    name: 'investment_range',
    eyebrow: 'Tu inversión',
    title: '¿Qué rango de inversión tienes contemplado para la consultoría?',
    inputOptions: { required: 'Selecciona una opción' },
    options: [
      { value: 'less-than-25k',  label: 'Menos de $25,000' },
      { value: '25k-70k',        label: '$25,000 – $70,000' },
      { value: '70k-150k',       label: '$70,000 – $150,000' },
      { value: 'more-than-150k', label: 'Más de $150,000' },
      { value: 'undefined',      label: 'Aún no lo defino' },
    ],
    cols: 1,
  },
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (leadType === 'qualified') {
        ...
        await router.push(`${WHATSAPP_URL}?text=${msg}`);
      } else {
